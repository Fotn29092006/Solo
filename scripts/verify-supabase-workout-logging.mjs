...
if (!supabaseUrl || !serviceRoleKey) {
  ...
  if (!reachability.ok && reachability.status === "network_error") {
    ...
    console.log("Checking Supabase workout logging table...");

    const workoutLogs = await probeEndpoint({
      baseUrl: supabaseUrl,
      serviceRoleKey,
      path: "/rest/v1/workout_logs?select=*&limit=1"
    });
    const workoutDetail = workoutLogs.error ? ` (${workoutLogs.error})` : "";

    console.log(`workout_logs: ${workoutLogs.status}${workoutDetail}`);

    if (!workoutLogs.ok) {
      console.error(
        "Workout logging verification failed. Apply supabase/migrations/0003_workout_logging.sql to the target project, then rerun this verifier."
      );
      process.exitCode = 1;
    } else {
      console.log("Workout logging verification passed.");
    }
  }
}
