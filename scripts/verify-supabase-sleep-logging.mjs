...
if (!supabaseUrl || !serviceRoleKey) {
  ...
  if (!reachability.ok && reachability.status === "network_error") {
    ...
    console.log("Checking Supabase sleep logging table...");

    const sleepLogs = await probeEndpoint({
      baseUrl: supabaseUrl,
      serviceRoleKey,
      path: "/rest/v1/sleep_logs?select=*&limit=1"
    });
    const sleepDetail = sleepLogs.error ? ` (${sleepLogs.error})` : "";

    console.log(`sleep_logs: ${sleepLogs.status}${sleepDetail}`);

    if (!sleepLogs.ok) {
      console.error(
        "Sleep logging verification failed. Apply supabase/migrations/0004_sleep_logging.sql to the target project, then rerun this verifier."
      );
      process.exitCode = 1;
    } else {
      console.log("Sleep logging verification passed.");
    }
  }
}
