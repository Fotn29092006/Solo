...
- Completed: switch default `npm run dev` to Next.js webpack dev mode for Telegram WebView compatibility; `npm run dev:turbo` remains available for browser-only preview work.
- Completed: add a development-only `Telegram bridge fallback` capture path that does not depend on React hydration and can copy real SDK `initData` or real Telegram `tgWebAppData` launch data.
- Next: restart `npm run dev`, reconnect ngrok, reopen the Mini App through Telegram, use the lower `Telegram bridge fallback` block if React remains unhydrated, capture fresh `TELEGRAM_TEST_INIT_DATA` if available, set `NEXT_PUBLIC_APP_URL=https://flashing-hazelnut-scored.ngrok-free.dev`, run `npm run smoke:telegram`, then choose the next explicit matcher candidate for sleep or meal.
